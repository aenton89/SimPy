...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::Python()
{
    ...
    if (!inited)
    {
        ...
        static const char* const identifiers[] = {
            ...
            "str","sum","super","tuple","type","vars","zip","__import__","format","compile","exit","quit"
        };
        for (auto& k : identifiers)
        {
            Identifier id;
        ...
        // --- Rozpoznawanie specjalnych metod magicznych ---
        static const char* const magicMethods[] = {
            "__init__", "__str__", "__repr__", "__len__", "__getitem__", "__setitem__",
            "__delitem__", "__iter__", "__next__", "__enter__", "__exit__", "__call__"
        };
        for (auto& k : magicMethods)
        {
            Identifier id;
            id.mDeclaration = "Magic method";
            langDef.mIdentifiers.insert(std::make_pair(std::string(k), id));
        }

        // --- Reguły tokenizacji ---
        langDef.mTokenRegexStrings.push_back({R"(\"\"\".*?\"\"\")", PaletteIndex::String});  // triple double-quote string
        langDef.mTokenRegexStrings.push_back({R"(\'\'\'.*?\'\'\')", PaletteIndex::String});  // triple single-quote string
        langDef.mTokenRegexStrings.push_back({R"(L?\"(\\.|[^\\\"])*\")", PaletteIndex::String});
        langDef.mTokenRegexStrings.push_back({R"(\'(\\.|[^\\'])*\')", PaletteIndex::String});
        langDef.mTokenRegexStrings.push_back({R"([+-]?([0-9]+(\.[0-9]*)?|\.[0-9]+)([eE][+-]?[0-9]+)?)", PaletteIndex::Number});
        langDef.mTokenRegexStrings.push_back({R"(@[a-zA-Z_][a-zA-Z0-9_]*)", PaletteIndex::Preprocessor}); // dekoratory
        langDef.mTokenRegexStrings.push_back({R"([a-zA-Z_][a-zA-Z0-9_]*)", PaletteIndex::Identifier});
        langDef.mTokenRegexStrings.push_back({R"([+\-*/%=!&|^~<>@:,.;?\\[\]{}()])", PaletteIndex::Punctuation});
        langDef.mTokenRegexStrings.push_back({R"(//|==|!=|<=|>=|\*\*|:=)", PaletteIndex::Punctuation});
